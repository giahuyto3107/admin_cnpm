...
function detailProduct(index) {
    ...
    <div class="notebox">
            ...
            <textarea class="text-note" id="popup-detail-note" placeholder="Hãy nhập size giày kèm các note liên quan...."></textarea>
    </div>
    ...
    if (localStorage.getItem('currentuser') != null) {
        ...
        if (currentuser.cart.length != 0) {
            ...
            currentuser.cart.forEach(item => {
                ...
                <p class="product-note"><i class="fa-solid fa-pen-to-square"></i><span>${product.note}</span></p>
...
function handleSearch(event) {
    ...
    // Cập nhật danh sách sản phẩm lọc hiện tại
    currentFilteredProducts = results;

    // Tính số trang mới dựa trên số sản phẩm lọc được
    totalPages = calculateTotalPages(results);

    // Hiển thị sản phẩm trang đầu tiên và cập nhật nút phân trang
    currentPage = 1;
    displayProductsOnPage(currentPage, currentFilteredProducts);
...
function kiemtradangnhap() {
    ...
    if (currentUser != null) {
        ...
        document.querySelector('.header-middle-right-menu').innerHTML = `
            <li><a href="javascript:;" onclick="orderHistory()"><i class="fa-solid fa-cart-shopping"></i>Đơn hàng đã mua</a></li>
            <li class="border"><a id="logout" href="javascript:;"><i class="fa-solid fa-right-from-bracket"></i>Đăng xuất</a></li>`
...
function checkAdmin() {
    ...
    if(user && user.userType == 1) {
        ...
        node.innerHTML = `<a href="./admin.html"><i class="fa-solid fa-shop"></i>Quản lý cửa hàng</a>`
...
function renderOrderProduct() {
    ...
    if (arrDonHang.length == 0) {
        orderHtml = `<div class="empty-order-section"><i class="fa-solid fa-spinner"></i><p>Chưa có đơn hàng nào</p></div>`;
    } else {
...
function detailOrder(id) {
    ...
    let detailOrderHtml = `<ul class="detail-order-group">
        <li class="detail-order-item">
            <span class="detail-order-item-left"><i class="fa-regular fa-calendar"></i>Ngày đặt hàng</span>
            <span class="detail-order-item-right">${formatDate(detail.thoigiandat)}</span>
        </li>
        <li class="detail-order-item">
            <span class="detail-order-item-left"><i class="fa-regular fa-user"></i>Người nhận</span>
            <span class="detail-order-item-right">${detail.nguoinhan}</span>
        </li>
        <li class="detail-order-item">
        ...
        <li class="detail-order-item">
            <span class="detail-order-item-left"><i class="fa-solid fa-location"></i>Địa chỉ nhận</span>
            <span class="detail-order-item-right">${detail.diachi}</span>
        </li>
    </ul>`
...
function renderProducts(showProduct) {
    ...
    if(showProduct.length == 0) {
        ...
        productHtml = `<div class="no-result"><div class="no-result-h">Không tìm thấy kết quả phù hợp</div><div class="no-result-p">Tìm cái khác giùm đi huhu!</div><div class="no-result-i"><i class="fa-regular fa-face-sad-cry"></i></div></div>`;
    } else {
            ...
            <article class="card-product" >
                ...
                <div class="shoes-info">
                    ...
                    <div class="product-buy">
                        <button onclick="detailProduct(${product.id})" class="card-button order-item"><i class="fa-solid fa-bag-shopping"></i>Đặt ngay</button>
                    </div> 
                </div>
                </div>
            </article>
